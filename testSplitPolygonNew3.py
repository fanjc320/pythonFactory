...
def is_concave(polygon, index):
    ...
    print("is_concave index:", index, " p1:", p1, " p2:", p2, " p3:", p3, " cross:", cross)
    # 对于逆时针多边形，叉积为负表示凹点
    # return cross < 0
    return cross > 0
...
def find_concave_points(polygon):
    """找到多边形中所有的凹点"""
    concave_indices = []
    points = list(polygon.exterior.coords)[:-1] # https://blog.csdn.net/weixin_44350337/article/details/116034510
    for i in range(len(points)):
        if is_concave(polygon, i):
            concave_indices.append(i)
    print("find_concave_points:", concave_indices, " polygon:", polygon)
...
def decompose_polygon(polygon):
    ...
    print("decompose_polygon, diagonals:", diagonals)
    # 存储所有可能的拆解方案
    decompositions = []
    ...
    for r in range(1, max_splits + 1):
        for combo in combinations(diagonals, r):
            # 尝试分割多边形
            print("decompose_polygon combo:", combo, " diagonals:", diagonals)
            sub_polygons = split_polygon(polygon, combo)
            print("decompose_polygon numb:", len(sub_polygons))
            # 检查分割结果是否有效
